...
class APIKeyAuthStrategy:
	def __init__(A,project_dir:Path,api_base_url:str=_B):B=project_dir;A.project_dir=B;A.api_base_url=api_base_url.rstrip('/');A.api_key_path=B/_C;A.session=requests.Session();A.name='API Key'
	def get_credentials(A)->Optional[str]:
		C=os.getenv('VNSTOCK_API_KEY')
	...
	def verify_credentials(B,credential:str)->Dict:
		U='deviceLimit';T='unknown';S='devicesUsed';R='machine_info';Q='os_version';P='os_type';O='device_name';N=credential;M='tier';I='device_id'
		try:
...
Please remove unused devices or upgrade your subscription.""";raise SystemExit(A)
	...
	def _get_username(A,api_key:str)->str:
		C='Unknown'
		try:
		...
		return C
def authenticate(project_dir:Path,api_base_url:str=_B)->Dict:
