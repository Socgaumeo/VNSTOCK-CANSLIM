...
def update_dynamic_sitemaps():
	L='last_sitemap_update';D='site_overrides';logger.info('Checking for sitemap updates...');H=sitemap_update_cache.get(L);I=datetime.now()
	if H:
...
async def fetch_news_from_site(site_name,max_articles,time_window):
	F='urls';D='rss';B=site_name;logger.info(f"Fetching news from {B}...");A=dynamic_config.get_sites_config().get(B)
...
def save_current_data(site_name,articles_df):
	E='utf-8-sig';B=site_name;A=articles_df
	if A.empty:return
...
def generate_report():
	...
	with open(N,'w',encoding=L)as A:
		A.write(f"News Monitor Report - {datetime.now().strftime("%Y-%m-%d %H:%M")}\n");A.write('='*50+'\n\n');A.write('STATISTICS\n');A.write(C*50+D);A.write(f"Total articles collected: {article_stats[_I]}\n");A.write(f"Articles in the last 24 hours: {len(E)}\n");A.write(f"New articles in last run: {article_stats[_C]}\n\n");A.write('BY SITE\n');A.write(C*50+D)
		for(O,F)in article_stats[_D].items():A.write(f"{O}: {F} articles\n")
		A.write('\nTRENDING TOPICS\n');A.write(C*50+D)
		for(P,(Q,F))in enumerate(J.items(),1):A.write(f"{P}. {Q}: {F} mentions\n")
		A.write('\nSITEMAP STATUS\n');A.write(C*50+D);R=dynamic_config.get_sites_config()
		for G in CONFIG[_F]:
			B=R.get(G,{})
			if _A in B:H=B[_A].get(_O,I);S=B[_A].get(_X,I);A.write(f"{G}: {H} (Type: {S})\n")
			else:H=B.get(_J,I);A.write(f"{G}: {H}\n")
	logger.info(f"Generated news report with {len(E)} recent articles");return K
async def fetch_all_news():
	E='%H:%M:%S';article_stats[_C]=0;logger.info(f"Starting news collection at {datetime.now().strftime(E)}");logger.info(f"Monitoring {len(CONFIG[_F])} sites with {CONFIG[_N]} articles per site");D=[]
...
def run_news_collection():asyncio.run(fetch_all_news())
def daily_maintenance():logger.info('Running daily maintenance...');update_dynamic_sitemaps();logger.info('Daily maintenance completed')
def main():
